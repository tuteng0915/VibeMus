...
def _history_preview_text(selected_display=None):
    ...
    if not target:
        target = entries[0]
    title = target.get('nickname') or target.get('label') or 'entry'
...
def run(message, history, prof, aout, lyr, tg, pth, nickname, history_selection):
    ...
    if new_aout and new_aout not in ('', 'blank.wav') and new_aout != pth:
        ...
        entry = log_audio_snapshot(new_aout, new_lyr, new_tg, nickname=nickname or None, chat_history=full_history)
        if entry:
            dropdown_update, hist_sel = _history_dropdown_update(entry['display'])
            summary_update = _history_summary_update()
            if nickname:
                nickname_update = gr.update(value='')
...
with gr.Blocks(css=custom_css) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=5):
            with gr.Accordion(label="History", open=True, elem_id="history-panel"):
                ...
                history_nickname = gr.Textbox(
                    label="Nickname for next save",
                    placeholder="Optional label to help identify this take",
                )
            with gr.Column(elem_id="chat-panel"):
                chatbot = gr.ChatInterface(
                    run,
                    type='messages',
                    additional_inputs=[profile, audio_output, lyrics, tags, path_name, history_nickname, history_selection_state],
                    additional_outputs=[lyrics, tags, audio_output, path_name, history_dropdown, history_summary, history_nickname, history_selection_state, history_preview, history_chatlog],
                    fill_height=True,
                )

    @generate_btn.click(
        inputs=[lyrics, tags, length, history_nickname, history_selection_state],
        outputs=[audio_output, path_name, history_dropdown, history_summary, history_nickname, history_selection_state, history_preview, history_chatlog],
    )
    def generate_music(lyr, tg, lth, nickname, history_selection):
        ...
        nickname = (nickname or '').strip()
        entry = log_audio_snapshot(outputs[0], lyr, tg, duration=lth, nickname=nickname or None)
        dropdown_update, hist_sel = _history_dropdown_update(entry['display'] if entry else history_selection)
        summary_update = _history_summary_update()
        nickname_update = gr.update(value='') if entry and nickname else gr.update()
        preview_update = _history_preview_update(hist_sel)
        chatlog_update = _history_chatlog_update(hist_sel)
        return outputs[0], outputs[0], dropdown_update, summary_update, nickname_update, hist_sel or '', preview_update, chatlog_update
    ...
    @restore_history_btn.click(inputs=[history_dropdown], outputs=[lyrics, tags, audio_output, path_name])
